...
## Abstract
We don't ask whether a program seems memory-safe after a few tests; we compile it against a type system and reject what violates it. AI agents now make governance decisions (what runs, what is remembered, when review is required, which instruction wins), yet we still certify those rules with a finite eval suite and hope. We should compile the governance rule itself against a typed legitimacy target, and reject, monitor, or sacrifice obligations before the rule governs durable state. A semantic legitimacy kernel is that target: a typed artifact bundling five separately load-bearing runtime obligations with graph diagnostics, a scaling witness, and an explicit bridge contract, inhabited by a concrete Lean instance, not merely defined. Two machine-checked theorems give it force: an obstructed rule may go live only if its forced sacrifices are declared, and a governed trajectory either preserves the kernel invariant or emits an indexed sacrifice certificate, both proved over the formal model in Lean 4 with no `sorry`, `admit`, or first-party `axiom`. A companion impossibility forces the design: a rule reaching a scarce, peer-relative stage cannot keep the full diagnostic package and must declare which obligation it sacrifices; on the constructed canonical claimant-interaction lift, definitionally `uniK5` with fixed signal `sig5`, a shared-substrate composition prices the same structure as a finite, computable capability ceiling. This join is not a per-input-graph spectral law. We reuse classical Arrow/Young/Thomson diagnostics; the novelty is the typed compiler target and bridge contract. A bounded, unverified Rust extractor turns real agent harnesses into graph models; current rejections are theorem-backed statements about those extracted graphs, not verdicts on live products.
## Highlights
...
> **One umbrella, one footprint.** The seven public spine statements — the spectral capacity threshold, the channel-capacity bound, the capability-scaling substrate, its shared cliff, the reachable peer-relative obstruction, the no-undeclared-sacrifice deployment gate, and the semantic-kernel decomposition — are tied together by a single umbrella theorem (`spineFootprint`) so that an axiom-footprint check cannot quietly cover only a selected subset of the spine. The kernel decomposition and the forcing obstruction sit in the same footprint by design.
...
The formal obstruction applies when a real permission decision reaches a scarce peer-relative stage. In the Claude Agent SDK gate, this is the situation where a tool-use request is evaluated against other active requests or review demands, not only against its own local features. On that reachable-stage class, the bundled graph-level legitimacy properties cannot all hold.
