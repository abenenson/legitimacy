...
## Abstract
An AI governance pipeline that transparently reaches a scarce, structurally peer-relative allocator and preserves its permits downstream cannot be consistent, solidary, and cross-claimant monotone at once. We prove this in machine-checked Lean; a companion safety theorem turns the obstruction into a deployment obligation, requiring a covered rule to declare its diagnostic sacrifice before deployment. The result transports the Arrow/Young impossibility lineage to a new substrate, the directed governance pipelines inside AI agents. An upstream deny-all gate supplies the countermodel that makes reachability necessary. In the permit/deny setting, strategyproofness follows from solidarity and monotonicity rather than entering as an independent assumption. On a constructed canonical claimant-interaction lift, a separate composition theorem prices the same obstruction as positive consistency vulnerability and hence a finite, computable capability ceiling. The artifact is verified end to end against a pinned Mathlib, with zero `sorry`, `admit`, or first-party `axiom`. Applied to graph models extracted from the OpenAI Codex hooks and the Claude Agent SDK, the diagnostic rejects both represented harnesses; each verdict is backed by a named theorem about the model, not the live product.
## Highlights

> **Main result.** An AI governance pipeline that transparently reaches a scarce, structurally peer-relative allocator and preserves its permits downstream cannot keep consistency, solidarity, and cross-claimant monotonicity together (§1.2).
