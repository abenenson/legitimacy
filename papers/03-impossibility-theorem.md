...
## Abstract
Scarce-resource governance rules can face unavoidable conflicts among consistency, solidarity, and cross-claimant monotonicity. We prove a machine-checked obstruction for claimant-symmetric, scarce-coupled allocators on complete first-effective binary decision pipelines. This wider class includes the median rule. A max-rule separator preserves consistency while violating monotonicity, showing why the general conclusion allows different sacrifices. Narrower structural peer-relative results force stronger individual failures, with reachability extensions specifying transparent prefixes and permit-preserving suffixes. A companion safety theorem requires the covered diagnostic sacrifices to be declared before activation. These results transport the Arrow/Young impossibility lineage to governance rules inside AI-agent models. In the permit/deny setting, strategyproofness follows from solidarity and monotonicity rather than entering as an independent assumption. On a constructed canonical claimant-interaction lift, a separate composition theorem joins an obstruction to positive consistency vulnerability and a finite, computable capability ceiling. The Lean statements are checked against a pinned Mathlib, with zero `sorry`, `admit`, or first-party `axiom`; their hypotheses and the source-to-model interpretation remain explicit. Applied to graph models extracted from the OpenAI Codex hooks and the Claude Agent SDK, the diagnostic rejects both represented harnesses; each verdict is backed by a named theorem about the model, not the live product.

## Highlights

> **Main result.** The median-inclusive class of claimant-symmetric, scarce-coupled allocators cannot preserve consistency, solidarity, and cross-claimant monotonicity together on complete first-effective binary pipelines. The narrower structural class supports stronger conclusions; the max-rule separator shows why those conclusions do not extend to the whole wider class (§2.1).
...
```{=latex}
\begin{tcolorbox}[figurebox,title=\textbf{Figure 1.} The reachable structural specialization]
\ttfamily\small
...
> **Two theorem tiers.** `symmetric_scarce_coupled_allocators_obstructed` is the wider obstruction: claimant symmetry, finite-estate coupling and a scarce admissible profile, together with the pipeline's effective-surface and complete-tail premises, rule out the three-property conjunction. The reachable structural theorem develops the narrower class through transparent prefixes and permit-preserving suffixes. Its additional rank laws support stronger individual failures. The median rule belongs to the wider class and is excluded from the narrower class; §2.1 gives the separation and necessity witnesses. In binary semantics, strategyproofness follows from solidarity plus monotonicity, so it is a derived property rather than a fourth independent axiom.
...
> **Counterexample boundary.** Bounded self-modification corrigibility is not characterized by RG-basin-A membership plus a stratification certificate on the extended kernel substrate: a concrete counterexample exhibits basin-A membership and a valid stratification certificate while failing bounded corrigibility (`RG_basin_stratification_corrigibility_biconditional_fails`). The result refutes that specific biconditional, not RG or corrigibility in general.
...
**Commons governance**. Ostrom's empirically grounded principles for robust commons institutions [Ostrom2010] concern boundary definition, participant rulemaking, monitoring, graduated sanctions, conflict resolution, and nested organization. The present theorem neither derives those principles nor equates them with its graph diagnostics. The framework instead makes a future comparison testable: represent a commons institution as a directed governance pipeline, then ask which diagnostics it satisfies or deliberately relaxes.
...
Legitimacy for peer-relative governance is best treated as a compiled, typed, auditable kernel rather than as a scalar verdict. The semantic target bundles five runtime obligations with graph diagnostics and a scaling witness through an explicit bridge contract. That bridge carries the diagnostic and spectral layers instead of inferring them from runtime checks alone. The bounded extractor contract then marks the operational boundary: well-formed bounded source inputs must compile to artifacts satisfying the semantic target.

The impossibility theorem explains why the kernel needs those diagnostic layers. When a transparent prefix reaches a nontrivial symmetric scarce peer-relative allocator and the suffix preserves its permits, consistency, solidarity, and cross-claimant monotonicity cannot coexist. Strategyproofness follows as a fourth corollary rather than entering as an independent assumption. A deny-all prefix supplies the countermodel that makes reachability necessary. Under the governance coverage contract, avoiding peer relativity leads to substantive collapse, concurrent-claim infeasibility, or runtime-kernel exit rather than a free escape.

The companion scaling results ask what a represented governance graph can still certify as capability grows. On the constructed canonical claimant-interaction lift, definitionally `uniK5` with fixed signal `sig5`, `capability_scaling_shared_cliff` converts the peer-relative obstruction into positive consistency vulnerability and a finite critical-capability cliff. That lift supplies the composition theorem's spectral carrier; separate worked lanes compute vulnerability on graph-derived carriers. The two papers therefore meet on one proved substrate.

The practical consequence is a sharper question: not simply "is this system aligned?" but "does this governance rule compile to a semantic legitimacy kernel, and what sacrifices does that kernel expose?" The extracted Codex graph, the modeled OpenClaw override case, and the AARs case study occupy distinct evidence lanes; each illustrates a modeled structural failure class without becoming a theorem about a live product. The Declared Sacrifice Protocol turns that diagnosis into an operational requirement: classify the sacrifice, bound it, and monitor it.

The impossibility theorem makes the compiler's diagnostic layers non-optional. A legitimate rule must expose the sacrifice it cannot avoid.
...
[Ostrom2010] Ostrom, E. Beyond Markets and States: Polycentric Governance of Complex Economic Systems. American Economic Review, 100(3):641–672, 2010. doi:10.1257/aer.100.3.641.
