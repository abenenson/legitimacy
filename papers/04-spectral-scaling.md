...
## Abstract
As an AI agent's modeled capability grows without bound, which governance rules can still certify its decisions, and which collapse? Every rule that certifies an agent's decisions has a zero-error capacity ceiling, computable from its own spectral structure: above the graph-side threshold C*(G,s,δ) = δ/cv(G,s), with cv the rule's consistency vulnerability, no certifier stays error-free as capability scales, and the ceiling is tight, so this is a genuine capacity theorem, not a one-sided impossibility. Stability obeys the same line: a rule sustains stability at unbounded capability exactly when its consistency vulnerability is zero; otherwise it has a finite, computable cliff, even against adversarial capability paths. The result is asymptotic and ASI-scale: the stability characterization holds at arbitrarily large capability, not only in bounded regimes. The vulnerability is forced, not accidental: a shared-substrate composition ties this cliff to the companion impossibility, whose scarce, peer-relative structure provably forces positive consistency vulnerability. That join is a theorem about the constructed canonical claimant-interaction lift, definitionally `uniK5` with fixed signal `sig5`, whose forced `cv` is `3/4`, not a per-input-graph spectral law. And below the ceiling the substrate does more than fail: it certifies, with one constructive procedure spanning constitutional-AI, adversarial, and AI-control settings at an audited scale. We work it on the extracted OpenAI Codex hooks harness, whose consistency vulnerability is one, a finite cliff an explicit repair removes. Two honest boundaries hold. C* is a property of the rule, not the Shannon capacity of a fixed channel and not a behavioral estimate of model capability; and the theorems bind to the represented graph, so RLHF, Constitutional AI, debate, and AI Control inherit nothing without separate embeddings. All machine-checked in Lean 4, with no `sorry`, `admit`, or first-party `axiom`.
## Highlights
