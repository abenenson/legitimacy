...
pub(super) fn module_target(root: &Path, source: &Path, module: &syn::ItemMod) -> PathBuf {
        ...
        .find(|attr| attr.path().is_ident("path"))
    {
        let syn::Meta::NameValue(value) = &path.meta else {
            panic!("path attribute must be name-value");
        };
        let syn::Expr::Lit(syn::ExprLit {
            ...
            panic!("path attribute value must be a string");
        };
        return normalize(&parent.join(value.value()));
    }
    let file = parent.join(format!("{}.rs", module.ident));
    if file.exists() {
        file
    } else {
...
pub(super) fn include_literals(source: &str) -> Vec<String> {
    ["include_bytes!(\"", "include!(\""]
        .into_iter()
        .flat_map(|prefix| {
            source.match_indices(prefix).map(move |(offset, _)| {
