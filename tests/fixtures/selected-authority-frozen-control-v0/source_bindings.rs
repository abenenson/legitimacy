...
use legitimacy::trajectory::codex_exec_v0::{
    ...
    CODEX_EXEC_FIXTURE_SPEC_BINDING_CONTRACT_V0, CODEX_EXEC_SANITIZER_BINDING_CONTRACT_V0,
    codex_exec_adapter_binding_v0, codex_exec_cli_publication_binding_v0,
...
pub(super) fn cli_production_rust_paths() -> Vec<String> {
    ...
    ["binary", "library"]
        ...
        .filter_map(|path| {
            ...
            path.ends_with(".rs").then(|| path.to_string())
        })
        .collect()
}
...
pub(super) fn canonical_build_input() -> serde_json::Value {
    serde_json::from_slice(include_bytes!(
        "../fixtures/codex-exec-v0/adapter-build-input.json"
    ))
    .unwrap()
}

pub(super) fn bound_input_paths(build: &serde_json::Value) -> Vec<String> {
...
fn canonical_build_input_binds_complete_selected_inputs() {
    ...
    assert_eq!(
        build_json["package"],
        serde_json::json!({"name":"legitimacy","version":"1.0.0","edition":"2024"})
    );
    ...
    let paths = bound_input_paths(&build_json);
