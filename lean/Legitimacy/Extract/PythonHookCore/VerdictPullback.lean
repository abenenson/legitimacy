...
/-!
# PythonHookCore verdict pullback

This module proves the pass-direction pullback currently supported by the
modeled PythonHookCore extractor: graph-level binary decision-system diagnostics
...
`AuditGovernanceGraph` data while PythonHookCore extraction
...
/-- Source-level certifiability for the modeled Python hook core: the source has
a nonempty registration table, every registered callback resolves, and the
...
def SourceCertifiability (source : Program) : Prop :=
  source.registrations ≠ [] ∧
    (∀ registration ∈ source.registrations,
      (findCallback? source.callbacks registration.callback).isSome) ∧
...
/-- Source-level observable determinacy for the modeled Python hook core:
    ...
    registrations.Perm source.registrations →
      ∀ (profile : ClaimProfile) (subject : ClaimantId),
        evalRegistrations source registrations profile subject =
          evalPythonHookCore source profile subject

/-- Source-level corrigibility projection for the modeled Python hook core:
each terminal hook result is explicitly present in a registered callback body.
This records source override surface, not the production extracted-graph
...
def SourceCorrigibilityProjection (source : Program) : Prop :=
  source.registrations ≠ [] ∧
    ∀ result : HookResult,
      ∃ registration ∈ source.registrations,
        ∃ callback,
          findCallback? source.callbacks registration.callback = some callback ∧
            callback.body = Stmt.returnDecision result

/-- Source-level compositional-safety projection for the modeled Python hook
core: every source registration has a resolvable callback and the source has at
...
/-- Source-level nonvacuity for the modeled Python hook core: some named
claimant in a concrete profile is permitted by the source semantics. -/
      ...
      | ok node =>
          ...
          | ok nodes =>
              ...
              | head =>
                ...
                cases hcb : findCallback? source.callbacks registration.callback with
                | none =>
                    simp [hcb] at hnode
                | some callback =>
                    simp
...
lemma extractPythonHookCore_length
...
lemma sourceRegistrations_nonempty_of_extractPythonHookCore_graph_nonempty
    (source : Program) (graph : GovernanceGraph)
    (hextract : extractPythonHookCore source = .ok graph)
    (hgraph : graph ≠ []) :
    source.registrations ≠ [] := by
  intro hnil
  have hlen := extractPythonHookCore_length source graph hextract
  rw [hnil] at hlen
  cases graph with
  | nil =>
      exact hgraph rfl
...
lemma sourceCompositionalSafetyProjection_of_extractPythonHookCore
    (source : Program) (graph : GovernanceGraph)
    (hextract : extractPythonHookCore source = .ok graph) :
...
modeled Python hook core. The first four constructors are exactly the generic
...
production audit-subject surface. The audit node payload is intentionally
canonical data; the behavioral connection used by pullback theorems is the
separate extracted-graph equivalence theorem. -/
...
/-- A PythonHookCore audit subject generated from a modeled source program.
The witness keeps the data-graph audit boundary explicit: `auditCheckStatus`
runs on `AuditSubject`, while the reflected predicates live on the source
program. -/
class GeneratedAuditSubject
    (source : Program) (subject : AuditSubject) where
  graph : GovernanceGraph
  extracted : extractPythonHookCore source = .ok graph
...
through `extractPythonHookCore_decision_equivalent`. -/
...
/-- Pass-direction pullback for the currently supported modeled PythonHookCore
...
theorem python_hook_kernel_check_reflects_source_protocol_satisfaction
    (source : Program)
    (graph : GovernanceGraph)
    (check : AuditCheck)
    (hextract : extractPythonHookCore source = .ok graph)
...
theorem python_hook_audit_holds_certifiability_reflects_source
    (source : Program) (graph : GovernanceGraph)
    (hextract : extractPythonHookCore source = .ok graph)
  ...
  rcases hholds with ⟨hgraph_nonempty, claims, claimant,
    happears, hpermit⟩
  refine ⟨?_, ?_, ?_⟩
  · exact sourceRegistrations_nonempty_of_extractPythonHookCore_graph_nonempty
      source graph hextract hgraph_nonempty
  · exact
      (sourceCompositionalSafetyProjection_of_extractPythonHookCore
        source graph hextract).2
  · refine ⟨claims, claimant, happears, ?_⟩
    have hequiv := extractPythonHookCore_decision_equivalent source graph hextract
    exact (hequiv claims claimant).trans hpermit

theorem python_hook_extraction_implies_source_compositional_safety
    (source : Program) (graph : GovernanceGraph)
    (hextract : extractPythonHookCore source = .ok graph) :
    SourceCompositionalSafetyProjection source := by
  exact sourceCompositionalSafetyProjection_of_extractPythonHookCore
    source graph hextract

theorem python_hook_audit_holds_nonvacuity_reflects_source
    (source : Program) (graph : GovernanceGraph)
    (hextract : extractPythonHookCore source = .ok graph)
  ...
  · have hequiv :=
      extractPythonHookCore_decision_equivalent source graph hextract
...
theorem GeneratedAuditSubject.reflects
  ...
  have hextract : extractPythonHookCore source = .ok graph :=
  ...
  cases check
  · exact python_hook_kernel_check_reflects_source_protocol_satisfaction
...
modeled PythonHookCore source semantics. The `GeneratedAuditSubject` witness is
the theorem-facing bridge between the extracted `AuditSubject` data graph and
the source program. The conclusion is graph-diagnostic only: the five kernel
diagnostic constructors are properties of the induced governance structure and do
not follow from the modeled source syntax plus graph-decision equivalence. -/
theorem python_hook_audit_pass_reflects_source_protocol_satisfaction
    (source : Program)
...
/-- Worked constructor-coverage surface for the PythonHookCore audit pullback:
...
check. -/
theorem python_hook_audit_pass_reflects_source_protocol_satisfaction_supported_checks
    (source : Program)
    (subject : AuditSubject)
    [GeneratedAuditSubject source subject] :
  ...
  exact python_hook_audit_pass_reflects_source_protocol_satisfaction
    source subject check hsupported hpass

/-- A one-callback source fixture used as a direct source-semantics example. -/
def permitOnlyCallback : CallbackDecl where
  name := "permit"
  body := Stmt.returnDecision HookResult.allow

/-- A minimal PythonHookCore program that permits every modeled claimant. -/
def permitOnlyProgram : Program :=
  Program.mk [] [] [permitOnlyCallback]
    [{ hook := "pre_tool_use", callback := "permit" }]
...
theorem permitOnlyProgram_source_consistency :
  ...
  change evalPythonHookCore permitOnlyProgram claims k = BinaryDecision.Deny at hden
  simp [permitOnlyProgram, permitOnlyCallback, Program.registrations,
    Program.callbacks, evalPythonHookCore, evalRegistrations, evalRegistration,
    findCallback?, Stmt.evalDecision, Stmt.evalHookResult, HookResult.toDecision]
    at hden
